...
export default function Workspace() {
  ...
  return <main>
    ...
    <section className="intro"><div><p className="eyebrow">01 / THE STARTER</p><h1>Make the conversation<br /><em>interactive.</em></h1><p className="description">A voice agent that can read your code. Your assignment is to help it do more.</p></div><div className="requirements"><span>YOUR THREE FEATURES</span><p>01 <strong>Highlight code</strong></p><p>02 <strong>Run code</strong></p><p>03 <strong>Propose edits</strong></p></div></section>
    <section className="problem"><span className="eyebrow">SAMPLE PROBLEM</span><p>{QUESTION}</p></section>
    <div className="workspace"><section className="panel"><div className="panel-head"><strong>solution.ts</strong><span>TypeScript · revision {revision}</span></div><Editor initialCode={STARTER_CODE} onChange={code => { documentRef.current = { ...documentRef.current, code, revision: documentRef.current.revision + 1 }; setRevision(documentRef.current.revision); }} /><div className="panel-foot">Highlighting, execution, and edit proposals are the assignment. This starter only reads editor content.</div></section>
